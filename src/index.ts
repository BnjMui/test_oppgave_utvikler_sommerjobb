...
const search = async (apiKey: string, query: string) => {
  ...
  try {
    const response = await fetch(
      `${braveApiBaseUrl}?${searchParams.toString()}`,
      {
        method: "GET",
        headers: {
...
server.tool(
    ...
    {
        query: z.string().min(1, "Query must not be empty").max(400, "Query must not be longer than 100 characters").refine(
            (val) => val.split(" ").length <= 50,
            "Query must not contain more than 10 words"
        ),
    },
    async ({query}) => {
